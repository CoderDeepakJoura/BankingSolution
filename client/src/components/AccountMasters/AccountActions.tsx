...
const AccountOperations: React.FC<AccountOperationsProps> = ({
  ...
  const OperationButton: React.FC<{
    ...
    >
      ...
      <span className="mb-2 text-blue-600 group-hover:text-blue-800 transition-colors duration-200 relative z-10">
        {icon}
      </span>{" "}
      {/* Icon always blue, above overlay */}
      <span className="text-sm font-medium relative z-10">{label}</span>
    </button>
  );

  return (
    <div className="bg-white p-7 rounded-xl shadow-lg border border-gray-100 transition-all duration-300 hover:shadow-xl">
      {" "}
      {/* Enhanced container */}
      <h2 className="text-xl font-bold text-gray-800 mb-5 border-b pb-3 border-gray-200">
        {accountType}
      </h2>{" "}
      {/* Stronger title with separator */}
      <div className="grid grid-cols-2 sm:grid-cols-2 gap-4">
        ...
        {prematureFdPath && (
          <OperationButton
            icon={<Eye size={22} />}
            label={`Premature ${accountType}`}
            onClick={() => navigate(prematureFdPath ?? "")}
          />
        )}
        {matureFdPath && (
          <OperationButton
            icon={<Eye size={22} />}
            label={`Mature/Renew ${accountType}`}
            onClick={() => navigate(matureFdPath ?? "")}
          />
        )}
