...
// Loan balance: positive = Dr (outstanding), negative = Cr (overpayment)
    ...
    <table className="w-full border-collapse text-xs">
      <thead>
        <tr>
          ...
          <TH className="w-28 text-right">{data.isStand ? "Advancement (Dr)" : "Advancement / Interest (Dr)"}</TH>
          {data.isStand && <TH className="w-28 text-right">Int Dr</TH>}
          {data.isStand && <TH className="w-28 text-right">Int Cr</TH>}
          <TH className="w-28 text-right">Recovery (Cr)</TH>
      ...
      <tbody>
        ...
        {data.entries.length === 0 ? (
          <tr>
            <td colSpan={colCount} className="text-center py-10 text-slate-400 italic text-xs">
          ...
          <tr key={i} className={`hover:bg-blue-50/50 transition-colors duration-100 ${i % 2 === 0 ? "bg-white" : "bg-slate-50/70"}`}>
            ...
            <TD className="text-slate-800">
              {entry.particulars}
              {longNar && entry.narration && <span className="text-slate-400"> &mdash; {entry.narration}</span>}
            </TD>
            <TD className="text-right text-red-700 font-medium">{entry.dr != null ? fmt(entry.dr) : ""}</TD>
            {data.isStand && <TD className="text-right text-orange-700 font-medium">{entry.intDr != null ? fmt(entry.intDr) : ""}</TD>}
            {data.isStand && <TD className="text-right text-teal-700 font-medium">{entry.intCr != null ? fmt(entry.intCr) : ""}</TD>}
      ...
      <tfoot>
        <tr>
          ...
          {data.isStand && <TDF className="text-right text-orange-700">{fmt(totalIntDr)}</TDF>}
          {data.isStand && <TDF className="text-right text-teal-700">{fmt(totalIntCr)}</TDF>}
...
const buildExportConfig = (data: LoanLedger, longNar: boolean): ExportConfig => {
  const spanCount = data.isStand ? 9 : 7;
  const columns = data.isStand ? [
    ...
    { header: "Particulars",       widthRatio: 0.28, align: "left"   as const },
    { header: "Advancement (Dr)",  widthRatio: 0.11, align: "right"  as const },
    { header: "Int Dr",            widthRatio: 0.10, align: "right"  as const },
    { header: "Int Cr",            widthRatio: 0.10, align: "right"  as const },
    { header: "Recovery (Cr)",     widthRatio: 0.11, align: "right"  as const },
    ...
    { header: "Particulars",               widthRatio: 0.36, align: "left"   as const },
    { header: "Advancement/Interest (Dr)", widthRatio: 0.14, align: "right"  as const },
    { header: "Recovery (Cr)",             widthRatio: 0.14, align: "right"  as const },
  ...
  return {
    meta: {
      ...
      reportTitle: `Loan Ledger -- ${data.accountNumber} ${data.accountName} | ${fmtShort(data.fromDate)} to ${fmtShort(data.toDate)}`,
      fileName: `LoanLedger_${data.accountNumber}_${isoDatePart(data.fromDate)}_${isoDatePart(data.toDate)}`,
      landscape: true,
    },
    columns, rows,
  };
...
const buildPrintHTML = (data: LoanLedger, longNar: boolean): string => {
  ...
  data.entries.forEach((e) => {
    sno++;
    const par = longNar && e.narration ? `${e.particulars} — ${e.narration}` : e.particulars;
    const intDrCell = isStand ? `<td class="amt int-dr">${e.intDr != null ? fmt(e.intDr) : ""}</td>` : "";
    const intCrCell = isStand ? `<td class="amt int-cr">${e.intCr != null ? fmt(e.intCr) : ""}</td>` : "";
    rows += `<tr class="${sno % 2 === 0 ? "even" : ""}"><td style="text-align:center">${sno}</td><td style="text-align:center;white-space:nowrap">${fmtShort(e.voucherDate)}</td><td style="text-align:center">${e.voucherNo}</td><td>${par}</td><td class="amt dr">${e.dr != null ? fmt(e.dr) : ""}</td>${intDrCell}${intCrCell}<td class="amt cr">${e.cr != null ? fmt(e.cr) : ""}</td><td class="amt">${fmtBal(e.balance)}</td></tr>`;
  });
  const intDrTotalCell = isStand ? `<td class="amt int-dr">${fmt(totalIntDr)}</td>` : "";
  const intCrTotalCell = isStand ? `<td class="amt int-cr">${fmt(totalIntCr)}</td>` : "";
  rows += `<tr class="total-row"><td colspan="4" style="text-align:right">Total / Closing Balance</td><td class="amt dr">${fmt(data.totalDr)}</td>${intDrTotalCell}${intCrTotalCell}<td class="amt cr">${fmt(data.totalCr)}</td><td class="amt">${fmtBal(data.closingBalance)}</td></tr>`;
  const advHeader = isStand ? "Advancement" : "Advancement/Interest";
  return `<!DOCTYPE html><html><head><meta charset="utf-8"/><title>Loan Ledger</title><style>
    *{margin:0;padding:0;box-sizing:border-box;}body{font-family:Arial,sans-serif;font-size:11px;color:#000;padding:12px;}
    ...
    .amt{text-align:right;font-variant-numeric:tabular-nums;}.dr{color:#b91c1c;}.cr{color:#065f46;}.int-dr{color:#c2410c;}.int-cr{color:#0f766e;}
  ...
  <table><thead><tr>
    ...
    <th style="text-align:left">Particulars</th><th style="width:100px">${advHeader}</th>${extraCols}<th style="width:90px">Recovery</th><th style="width:90px">Balance</th>
  ...
  <div class="summary">
    ...
    <div class="summary-card" style="border-top-color:#ef4444"><div class="lbl">Total Advancement${isStand ? "" : " / Interest"}</div><div class="val" style="color:#b91c1c">&#8377;${fmt(data.totalDr)}</div></div>
    ${isStand ? `<div class="summary-card" style="border-top-color:#ea580c"><div class="lbl">Total Int Dr</div><div class="val" style="color:#c2410c">&#8377;${fmt(totalIntDr)}</div></div>` : ""}
    ${isStand ? `<div class="summary-card" style="border-top-color:#0d9488"><div class="lbl">Total Int Cr</div><div class="val" style="color:#0f766e">&#8377;${fmt(totalIntCr)}</div></div>` : ""}
    <div class="summary-card" style="border-top-color:#10b981"><div class="lbl">Total Recovery</div><div class="val" style="color:#065f46">&#8377;${fmt(data.totalCr)}</div></div>
...
const LoanLedgerPage: React.FC = () => {
  ...
  const [data, setData] = useState<LoanLedger | null>(null);
  ...
  useEffect(() => {
    ...
    loanLedgerApi.getLoanAccounts(user.branchid, selectedProduct).then((res) => {
  ...
  const handleShow = async () => {
    if (selectedProduct === "") { Swal.fire("Validation", "Please select a Loan Product.", "warning"); return; }
    ...
    try {
      const res = await loanLedgerApi.getLoanLedger(user.branchid, selectedAccount as number, effectiveFromDate, toDate);
          ...
          <div className="w-full space-y-5">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
              <div className="flex items-center gap-3 px-5 py-4 border-b border-slate-200">
                ...
                <div>
                  <h2 className="text-base font-bold text-slate-800">Loan Account Ledger</h2>
                  <p className="text-xs text-slate-500">Transaction history for a loan account</p>
                </div>
              </div>
              ...
              <div className="p-5 space-y-5">
                <div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    <div>
                      <label className={labelClass}>Loan Product <span className="text-red-500 normal-case">*</span></label>
                  ...
                  <div className="flex flex-wrap gap-4 items-end">
                    ...
                    {sessionMinDate && (
                      <div className="text-xs text-slate-400 pb-1">
                        Session: <span className="text-slate-600 font-medium">{fmtDate(sessionMinDate)}</span> &mdash; <span className="text-slate-600 font-medium">{fmtDate(sessionMaxDate)}</span>
                      </div>
                    )}
              ...
              <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                <div className="text-center px-6 py-6 bg-gradient-to-b from-slate-50 to-white border-b border-slate-200">
                  ...
                  <div className="flex items-center gap-3 justify-center mt-3">
                    ...
                    <span className="text-xs font-semibold uppercase tracking-widest text-slate-600 px-3 py-1 bg-blue-50 border border-blue-100 rounded-full">Loan Account Ledger</span>
                    <div className="h-px bg-slate-200 flex-1 max-w-16" />
                  </div>
                </div>
