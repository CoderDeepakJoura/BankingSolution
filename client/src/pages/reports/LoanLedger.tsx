  ...
  <table className="w-full border-collapse text-xs">
    <thead>
      <tr>
        ...
        <TH className="w-32 text-right">Advancement / Interest (Dr)</TH>
...
const buildExportConfig = (data: LoanLedger, longNar: boolean): ExportConfig => {
  ...
  return {
    meta: {
      ...
      reportTitle: `Loan Ledger â€” ${data.accountNumber} ${data.accountName} | ${fmtShort(data.fromDate)} to ${fmtShort(data.toDate)}`,
...
const buildPrintHTML = (data: LoanLedger, longNar: boolean): string => {
  ...
  <table><thead><tr>
    ...
    <th style="text-align:left">Particulars</th><th style="width:100px">Advancement/Interest</th><th style="width:90px">Recovery</th><th style="width:90px">Balance</th>
  ...
  <div class="summary">
    ...
    <div class="summary-card" style="border-top-color:#ef4444"><div class="lbl">Total Advancement / Interest</div><div class="val" style="color:#b91c1c">â‚¹${fmt(data.totalDr)}</div></div>
    <div class="summary-card" style="border-top-color:#10b981"><div class="lbl">Total Recovery</div><div class="val" style="color:#065f46">â‚¹${fmt(data.totalCr)}</div></div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    <div className="lg:col-span-2">
                      ...
                      <select value={selectedAccount} onChange={(e) => { setSelectedAccount(e.target.value === "" ? "" : Number(e.target.value)); setData(null); }} disabled={accounts.length === 0} className={selectClass}>
                        ...
                        {accounts.map((a) => <option key={a.id} value={a.id}>{a.accountNumber} â€” {a.accountName}</option>)}
                      </select>
                    </div>
                  </div>
                </div>
                ...
                <div className="px-5 py-4 bg-slate-50 border-b border-slate-200">
                  ...
                  {(data.loanAmount != null || data.loanDate || data.loanPeriodMonths != null || data.standardIntRate != null) && (
                    <div className="mt-2 pt-2 border-t border-slate-200 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-x-6 gap-y-2 text-xs">
                      {[
                        { label: "Loan Amount",      value: data.loanAmount != null ? `â‚¹${data.loanAmount.toLocaleString("en-IN")}` : null },
                ...
                <div className="p-4 sm:p-5">
                  ...
                  <div className="mt-5 grid grid-cols-2 sm:grid-cols-4 gap-4">
                    <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm border-t-4 border-t-blue-500">
                      ...
                      <p className="text-lg font-bold text-blue-700 mt-1">â‚¹{fmtBal(data.openingBalance)}</p>
                    </div>
                    <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm border-t-4 border-t-red-500">
                      ...
                      <p className="text-lg font-bold text-red-700 mt-1">â‚¹{fmt(data.totalDr)}</p>
                    </div>
                    <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm border-t-4 border-t-emerald-500">
                      ...
                      <p className="text-lg font-bold text-emerald-700 mt-1">â‚¹{fmt(data.totalCr)}</p>
                    </div>
                    <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm border-t-4 border-t-violet-500">
                      ...
                      <p className="text-lg font-bold text-violet-700 mt-1">â‚¹{fmtBal(data.closingBalance)}</p>
                    </div>
                  </div>
                </div>
              </div>
