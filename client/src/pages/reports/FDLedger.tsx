...
import fdLedgerApi, {
  ...
  FDLedger,
  FD_STATUS_LABELS,
} from "../../services/reports/fdLedgerApi";
...
// FD is a liability: positive = Cr (bank holds deposit), negative = Dr
...
const LedgerTable: React.FC<{ data: FDLedger; longNar: boolean }> = ({ data, longNar }) => (
...
const buildExportConfig = (data: FDLedger, longNar: boolean): ExportConfig => {
  const detailSuffix = data.selectedDetailLabel ? ` [${data.selectedDetailLabel}]` : " [All Details]";
  const columns = [
    ...
    { header: "Deposits",       widthRatio: 0.11, align: "right"  as const },
...
const buildPrintHTML = (data: FDLedger, longNar: boolean): string => {
  ...
  return `<!DOCTYPE html><html><head><meta charset="utf-8"/><title>FD Ledger</title><style>
  ...
  <table><thead><tr>
    ...
    <th style="text-align:left">Particulars</th><th style="width:90px">Withdrawals</th><th style="width:90px">Deposits</th><th style="width:90px">Int. Posted</th><th style="width:90px">Balance</th>
        ...
        <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 flex-1">
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-3 shadow-sm border-t-4 border-t-blue-500">
              ...
              <p className="text-base font-bold text-blue-700 mt-1">₹{fmt(ledger.totalInterestPosted ?? 0)}</p>
            </div>
...
const FDLedgerPage: React.FC = () => {
  ...
  const [accounts, setAccounts] = useState<FDAccountItem[]>([]);
  const [details, setDetails] = useState<FDDetailItem[]>([]);
  ...
  useEffect(() => {
    ...
    fdLedgerApi.getFDProducts(user.branchid).then((res) => {
      if (res.success && res.data) setProducts(res.data);
    }).catch(() => {});
  }, [user.branchid]);

  useEffect(() => {
    setData(null); setDetails([]); setSelectedAccount(""); setSelectedDetail("");
    if (!user.branchid || selectedProduct === "") { setAccounts([]); return; }
    fdLedgerApi.getFDAccounts(user.branchid, selectedProduct as number).then((res) => {
      if (res.success && res.data) setAccounts(res.data);
    }).catch(() => {});
  }, [selectedProduct]);

  useEffect(() => {
    setData(null); setSelectedDetail(""); setDetails([]);
    if (!user.branchid || selectedAccount === "") return;
    fdLedgerApi.getFDDetails(user.branchid, selectedAccount as number).then((res) => {
  ...
  useEffect(() => {
    ...
    if (!det) return;
    const detFrom = toInputDate(det.fdDate);
    const detTo   = toInputDate(det.fdMaturityDate);
    setFromDate(detFrom);
    setToDate(detTo > workingDate ? workingDate : detTo);
    setTillDateOnly(false);
  }, [selectedDetail]);
  ...
  const handleShow = async () => {
    if (selectedProduct === "") { Swal.fire("Validation", "Please select an FD Product.", "warning"); return; }
          ...
          <div className="w-full space-y-5">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
              <div className="flex items-center gap-3 px-5 py-4 border-b border-slate-200">
                ...
                <div>
                  <h2 className="text-base font-bold text-slate-800">Fixed Deposit Ledger</h2>
                  <p className="text-xs text-slate-500">Transaction history for an FD account</p>
                </div>
              </div>
              ...
              <div className="p-5 space-y-5">
                ...
                <div>
                  ...
                  {selectedDetail !== "" && (() => {
                    const det = details.find((d) => d.id === selectedDetail);
                    if (!det) return null;
                    const statusLabel = FD_STATUS_LABELS[det.fdStatus] ?? String(det.fdStatus);
                    const badgeClass = STATUS_BADGE[det.fdStatus] ?? "bg-slate-100 text-slate-600";
                    return (
                      <div className="mt-3 flex flex-wrap gap-2 items-center">
                        <span className="text-xs text-slate-400 font-medium">Selected detail:</span>
                        <span className="inline-flex items-center gap-1.5 bg-white border border-slate-200 rounded-md px-2.5 py-1 text-xs shadow-sm">
                          ...
                          <span className="font-semibold text-slate-800">{fmtDate(det.fdDate)}</span>
                        </span>
                        <span className="inline-flex items-center gap-1.5 bg-white border border-slate-200 rounded-md px-2.5 py-1 text-xs shadow-sm">
                          <span className="text-slate-400">Maturity</span>
                          <span className="font-semibold text-slate-800">{fmtDate(det.fdMaturityDate)}</span>
                        </span>
                        <span className="inline-flex items-center gap-1.5 bg-white border border-slate-200 rounded-md px-2.5 py-1 text-xs shadow-sm">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-4 items-end">
                    ...
                    <div className="w-44">
                      ...
                      <DatePicker value={toDate} onChange={setToDate} min={sessionMinDate || undefined} max={workingDate} workingDate={workingDate} className={dateClass} />
                    </div>
                    {sessionMinDate && (
