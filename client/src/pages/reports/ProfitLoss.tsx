...
const sectionExportRows = (sections: ProfitLossSection[]): ExportRow[] => {
  ...
  sections.forEach(sec => {
    ...
    rows.push({ style: "subtotal", cells: [`Total â€” ${sec.typeName}`, fmt(sec.subTotal)] });
  });
  return rows;
};
...
const buildExportConfig = (report: ProfitLoss): ExportConfig => {
  const columns = [
    ...
    { header: "Amount (â‚¹)",            widthRatio: 0.25, align: "right" as const },
  ];
  const rows: ExportRow[] = [];
...
// â”€â”€ Print HTML â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
const buildPrintHTML = (report: ProfitLoss): string => {
  const secHtml = (sections: ProfitLossSection[]) =>
    sections.map(sec =>
      ...
      `<tr class="ss"><td>Total â€” ${sec.typeName}</td><td class="amt">${fmt(sec.subTotal)}</td></tr>`
    ).join("");
...
<div class="cols">
  <div class="col">
    <table><thead><tr><th colspan="2">Expenditure</th></tr></thead><tbody>
      ...
      <tr class="gt"><td>Total Expenditure</td><td class="amt">â‚¹${fmt(report.totalExpense + (report.netProfit > 0 ? report.netProfit : 0))}</td></tr>
    </tbody></table>
  </div>
  <div class="col">
    <table><thead><tr><th colspan="2">Income</th></tr></thead><tbody>
      ...
      <tr class="gt"><td>Total Income</td><td class="amt">â‚¹${fmt(report.totalIncome + (report.netProfit < 0 ? Math.abs(report.netProfit) : 0))}</td></tr>
    </tbody></table>
  </div>
</div>
<div class="summ">
  <span>Total Income: <strong style="color:#166534">â‚¹${fmt(report.totalIncome)}</strong></span>
  <span>Total Expense: <strong style="color:#991b1b">â‚¹${fmt(report.totalExpense)}</strong></span>
  <span style="border-left:1px solid #cbd5e1;padding-left:24px">Net ${report.netProfit >= 0 ? "Profit" : "Loss"}: <strong style="color:${report.netProfit >= 0 ? "#166534" : "#991b1b"}">â‚¹${fmt(Math.abs(report.netProfit))}</strong></span>
</div>
</body></html>`;
};
...
const SectionRows: React.FC<{ section: ProfitLossSection; showDrCr?: boolean }> = ({ section, showDrCr }) => (
  <>
    ...
    <tr className="bg-slate-100">
      <TD className="font-semibold text-slate-700">Total â€” {section.typeName}</TD>
...
const ProfitLossPage: React.FC = () => {
  ...
  const handleLoad = async () => {
    ...
    if (fromDate > toDate)    { Swal.fire("Validation", "From Date must be â‰¤ To Date.", "warning"); return; }
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
              ...
              <div className="grid grid-cols-3 gap-3 p-4 bg-slate-50 border-b border-slate-200">
                <div className="bg-white border border-slate-200 rounded-xl p-3 shadow-sm border-t-4 border-t-emerald-500 text-center">
                  ...
                  <p className="text-base font-bold text-emerald-700 mt-0.5">â‚¹{fmt(report.totalIncome)}</p>
                </div>
                <div className="bg-white border border-slate-200 rounded-xl p-3 shadow-sm border-t-4 border-t-red-500 text-center">
                  ...
                  <p className="text-base font-bold text-red-700 mt-0.5">â‚¹{fmt(report.totalExpense)}</p>
                </div>
                <div className={`bg-white border border-slate-200 rounded-xl p-3 shadow-sm border-t-4 ${report.netProfit >= 0 ? "border-t-blue-500" : "border-t-orange-500"} text-center`}>
                  ...
                  <p className={`text-base font-bold mt-0.5 ${report.netProfit >= 0 ? "text-blue-700" : "text-orange-700"}`}>â‚¹{fmt(Math.abs(report.netProfit))}</p>
                </div>
              </div>
                  ...
                  <table className="w-full border-collapse">
                    <thead>
                      <tr>
                        ...
                        {showDrCr && <TH className="text-right w-24 bg-slate-50 text-slate-600">Dr (â‚¹)</TH>}
                        {showDrCr && <TH className="text-right w-24 bg-slate-50 text-slate-600">Cr (â‚¹)</TH>}
                        <TH className="text-right w-28 bg-slate-50 text-slate-700">Amount (â‚¹)</TH>
                      </tr>
                    </thead>
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td className="border border-slate-300 bg-slate-800 px-3 py-2 text-xs font-bold text-white text-right tabular-nums">
                          â‚¹{fmt(report.totalExpense + (report.netProfit > 0 ? report.netProfit : 0))}
                        </td>
                      </tr>
                    </tbody>
                  </table>
                  ...
                  <table className="w-full border-collapse">
                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        {showDrCr && <td className="border border-slate-300 bg-slate-800" />}
                        <td className="border border-slate-300 bg-slate-800 px-3 py-2 text-xs font-bold text-white text-right tabular-nums">
                          â‚¹{fmt(report.totalIncome + (report.netProfit < 0 ? Math.abs(report.netProfit) : 0))}
                        </td>
                      </tr>
                    </tbody>
                  </table>
