...
const STATUS_COLORS: Record<string, string> = {
  ...
  "Pre-Matured": "bg-orange-50 text-orange-700",
  Renewed: "bg-slate-100 text-slate-600",
};

const buildExportConfig = (report: FDMaturity): ExportConfig => {
  const columns = [
    { header: "Acc No",        widthRatio: 0.10, align: "left"   as const },
    { header: "Account Name",  widthRatio: 0.18, align: "left"   as const },
    { header: "Product",       widthRatio: 0.12, align: "left"   as const },
    { header: "FD Date",       widthRatio: 0.09, align: "center" as const },
    { header: "Maturity Date", widthRatio: 0.09, align: "center" as const },
    { header: "Period",        widthRatio: 0.07, align: "center" as const },
    { header: "Rate %",        widthRatio: 0.06, align: "center" as const },
    { header: "FD Amount",     widthRatio: 0.10, align: "right"  as const },
    { header: "Mat. Amount",   widthRatio: 0.10, align: "right"  as const },
  ...
  const rows: ExportRow[] = report.rows.map(r => ({
    ...
    cells: [
      ...
      fmtShort(r.fdDate), fmtShort(r.maturityDate),
      r.periodMonths > 0 ? `${r.periodMonths}m` : `${r.periodDays}d`,
      String(r.intRate),
      fmt(r.fdAmount), fmt(r.maturityAmount), fmt(r.maturityAmount - r.fdAmount),
    ],
  }));
  rows.push({
    style: "total", spanFirst: 7,
    cells: ["Total", "", "", "", "", "", "", fmt(report.totalFDAmount), fmt(report.totalMaturityAmount), fmt(report.totalInterestAmount)],
  });
  return {
    meta: {
      title: report.branchName, subtitle: report.branchAddress,
      reportTitle: `FD Maturity | ${fmtShort(report.fromDate)} to ${fmtShort(report.toDate)}`,
      fileName: `FDMaturity_${toInput(report.fromDate)}_${toInput(report.toDate)}`,
      landscape: true,
    },
    columns, rows,
  };
...
const buildPrintHTML = (report: FDMaturity): string => {
  const statusBg: Record<string, string> = { Open: "#dcfce7", Matured: "#dbeafe", "Pre-Matured": "#ffedd5", Renewed: "#f1f5f9" };
  let rows = "";
  report.rows.forEach((r, i) => {
    const bg = statusBg[r.status] ?? "#fff";
    const period = r.periodMonths > 0 ? `${r.periodMonths}m` : `${r.periodDays}d`;
    const interest = fmt(r.maturityAmount - r.fdAmount);
    rows += `<tr style="background:${i % 2 === 0 ? "#fff" : "#f8fafc"}">
      ...
      <td style="text-align:center">${period}</td><td style="text-align:center">${r.intRate}%</td>
      <td class="amt">${fmt(r.fdAmount)}</td><td class="amt">${fmt(r.maturityAmount)}</td><td class="amt">${interest}</td>
      <td style="text-align:center"><span style="background:${bg};padding:1px 6px;border-radius:4px;font-size:9px;font-weight:600">${r.status}</span></td>
    </tr>`;
  });
  rows += `<tr class="total-row"><td colspan="7" style="text-align:right">Total</td><td class="amt">${fmt(report.totalFDAmount)}</td><td class="amt">${fmt(report.totalMaturityAmount)}</td><td class="amt">${fmt(report.totalInterestAmount)}</td><td></td></tr>`;
  return `<!DOCTYPE html><html><head><meta charset="utf-8"/><title>FD Maturity</title><style>
...
<div class="rh"><h1>${report.branchName}</h1><p>${report.branchAddress ?? ""}</p><h2>FD Maturity Report | ${fmtLong(report.fromDate)} to ${fmtLong(report.toDate)}${report.productName !== "All Products" ? ` | ${report.productName}` : ""}</h2></div>
<table><thead><tr>
  <th style="text-align:left">Acc No</th><th style="text-align:left">Account Name</th><th style="text-align:left">Product</th>
  <th>FD Date</th><th>Maturity Date</th><th>Period</th><th>Rate %</th>
  <th style="width:80px">FD Amount</th><th style="width:80px">Mat. Amount</th><th style="width:80px">Interest</th><th>Status</th>
...
const FDMaturityPage: React.FC = () => {
  ...
  const [report, setReport]     = useState<FDMaturity | null>(null);

  useEffect(() => {
    if (!user.branchid) return;
    fdMaturityApi.getFDProducts(user.branchid).then(res => {
  ...
  const handleLoad = async () => {
    ...
    try {
      const res = await fdMaturityApi.getFDMaturity(user.branchid, fromDate, toDate, productId);
        ...
        <div className="max-w-7xl mx-auto space-y-5">
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
            <div className="flex items-center gap-3 px-5 py-4 border-b border-slate-200">
              ...
              <div><h2 className="text-base font-bold text-slate-800">FD Maturity Report</h2><p className="text-xs text-slate-500">Fixed deposits maturing in the selected date range</p></div>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
              <div className="text-center px-6 py-5 border-b border-slate-200 bg-gradient-to-b from-slate-50 to-white">
                ...
                <div className="flex items-center gap-3 justify-center mt-3">
                  ...
                  <span className="text-xs font-semibold uppercase tracking-widest text-amber-700 px-3 py-1 bg-amber-50 border border-amber-100 rounded-full">FD Maturity Report</span>
                  <div className="h-px bg-slate-200 flex-1 max-w-16" />
                </div>
                ...
                {report.rows.length === 0 ? (
                  ...
                  <div className="border border-slate-200 rounded-xl overflow-hidden shadow-sm overflow-y-auto max-h-[60vh]">
                    <table className="w-full border-collapse text-xs">
                      <thead>
                        <tr>
                          {["Acc No","Account Name","Product","FD Date","Maturity Date","Period","Rate %","FD Amount","Mat. Amount","Interest","Status"].map(h => (
                            <th key={h} className={`bg-slate-800 text-white px-3 py-3 text-xs font-semibold uppercase tracking-wider sticky top-0 z-10 border-r border-slate-700 last:border-r-0 ${["FD Amount","Mat. Amount","Interest"].includes(h) ? "text-right" : ["FD Date","Maturity Date","Period","Rate %","Status"].includes(h) ? "text-center" : "text-left"}`}>{h}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
                        {report.rows.map((r, i) => {
                          const interest = r.maturityAmount - r.fdAmount;
                          const period = r.periodMonths > 0 ? `${r.periodMonths}m` : `${r.periodDays}d`;
                          return (
                            <tr key={r.accountId} className={i % 2 === 0 ? "bg-white" : "bg-slate-50/70"}>
                              ...
                              <td className="border-b border-slate-100 px-3 py-2 text-center text-slate-600">{period}</td>
                              <td className="border-b border-slate-100 px-3 py-2 text-center text-slate-600">{r.intRate}%</td>
                              ...
                              <td className="border-b border-slate-100 px-3 py-2 text-right text-emerald-700 font-medium">{fmt(interest)}</td>
                              <td className="border-b border-slate-100 px-3 py-2 text-center">
                                <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${STATUS_COLORS[r.status] ?? "bg-slate-100 text-slate-600"}`}>{r.status}</span>
                              </td>
                            </tr>
                          );
                        })}
                      ...
                      <tfoot>
                        <tr>
                          ...
                          <td className="bg-slate-100 border-t-2 border-slate-400 px-3 py-2.5 text-right text-xs font-bold text-emerald-700">{fmt(report.totalInterestAmount)}</td>
