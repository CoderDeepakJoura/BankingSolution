...
const fetchAccountHeadTypes = async (filter: AccountHeadTypeFilter, branchId: number) => {
  ...
  return {
    ...
    data: res.accountHeadType ?? [], // Ensure 'totalCount' is a number, defaulting to 0.
...
`,
    ...
    preConfirm: () => {
      ...
      return { AccountHeadTypename, AccountHeadTypenamesl: (document.getElementById("AccountHeadTypenamesl") as HTMLInputElement).value.trim()};
    },
  });
  ...
  if (formValues) {
    try {
      ...
      Swal.fire("Error!", err.message || "Failed to add Account Head Type.", "error");
    }
  }
};
      ...
      <div class="flex items-start gap-2">
        ...
        <div class="text-xs text-slate-600">
          ...
          <ul class="list-disc list-inside mt-1 space-y-1 text-amber-600">
            <li>AccountHeadType code changes may affect existing records</li>
            <li>Ensure Account Head Type Name and AccountHeadType Code is unique in the system</li>
          </ul>
        </div>
      </div>
    </div>
...
`,
    ...
    preConfirm: () => {
      ...
      return { id: AccountHeadType.accountHeadTypeId, AccountHeadTypename, AccountHeadTypenamesl: (document.getElementById("AccountHeadTypenamesl") as HTMLInputElement).value.trim() };
    },
  });
