...
interface PledgedFDOption {
  pledgeId: number;
  fdAccId: number;
  fdAccDetId: number | null;
...
const UnpledgeFDPage: React.FC = () => {
  const user = useSelector((state: RootState) => state.user);
  const navigate = useNavigate();
  const today = commonservice.getTodaysDate();

  const [fromDate, setFromDate] = useState<string>(today);
  const [fdOptions, setFdOptions] = useState<{ value: number; label: string; data: PledgedFDOption }[]>([]);
  const [selectedOption, setSelectedOption] = useState<{ value: number; label: string; data: PledgedFDOption } | null>(null);
  ...
  const loadPledgedAccounts = async () => {
    ...
    setLoanBalance(null);
    try {
      const res = await commonservice.fetch_fd_accounts_pledged_locked(user.branchid, fromDate);
      const data: PledgedFDOption[] = (res as any)?.data ?? (res as any)?.Data ?? [];
      setFdOptions(
        data.map((item) => ({
          value: item.pledgeId,
  ...
  const handleFDSelect = async (opt: { value: number; label: string; data: PledgedFDOption } | null) => {
    ...
    const fd = opt.data;
    if (fd.status === PLEDGE_STATUS && fd.loanAccId) {
      setLoadingBalance(true);
      try {
        const balRes = await commonservice.get_account_balance(user.branchid, fd.loanAccId);
        const bal = (balRes as any)?.data ?? (balRes as any)?.Data ?? 0;
        // For loan accounts: Cr - Dr. If negative, loan outstanding. If >= 0, loan cleared.
  ...
  const handleAction = async (action: number) => {
    ...
    const confirm = await Swal.fire({
      icon: "question",
      title: `${actionLabel} FD?`,
      text: `Are you sure you want to ${actionLabel.toLowerCase()} FD ${selectedOption.data.accountNumber}?`,
    ...
    setSubmitting(true);
    try {
      const res = await commonservice.unpledge_unlock_fd({
        brId: user.branchid,
      ...
      if ((res as any)?.success || (res as any)?.Success) {
        await Swal.fire({
          icon: "success",
          title: "Success",
          text: `FD ${actionLabel.toLowerCase()}d successfully.`,
          timer: 2000,
        ...
        Swal.fire({
          icon: "error",
          title: "Failed",
          text: (res as any)?.message || (res as any)?.Message || `Failed to ${actionLabel.toLowerCase()} FD.`,
        });
      }
    } catch {
    ...
    <div className="-mt-3 bg-gradient-to-br from-gray-100 to-blue-50 p-4 sm:p-6 lg:p-8">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-gray-600 text-sm">Release pledged or locked Fixed Deposits</p>
            </div>
          </div>
          <button
            onClick={() => navigate(-1)}
      ...
      <div className="bg-white rounded-xl shadow border border-gray-200 p-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          {/* FD Account */}
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">FD Account</label>
            <Select
              options={fdOptions}
              value={selectedOption}
              onChange={handleFDSelect}
              isLoading={loadingAccounts}
              placeholder="Select pledged / locked FD..."
              isClearable
              classNamePrefix="react-select"
              noOptionsMessage={() => "No pledged/locked FDs found for this date"}
        ...
        {selected && (
          <div className="mt-6 border border-gray-200 rounded-lg p-5 bg-gray-50">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
              <div>
                <span className="text-gray-500">FD Account:</span>{" "}
