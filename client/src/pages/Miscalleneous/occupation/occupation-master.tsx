...
import OccupationApiService from "../../../services/Occupation/Occupationapi";
...
const OccupationMaster: React.FC = () => {
  ...
  const [OccupationNameSL, setOccupationNameSL] = React.useState("");
  const [error, setError] = React.useState("");
  const [loading, setLoading] = React.useState(false);

  const hindiRegex = /^[\u0900-\u097F\s.,!?]*$/;

  const handleOccupationNameSLChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const inputText = e.target.value;
    if (inputText === "" || hindiRegex.test(inputText)) {
      setOccupationNameSL(inputText);
      setError("");
    } else {
  ...
  const handleReset = () => {
    ...
    setLoading(false);
    const OccupationName = document.getElementById("OccupationName") as HTMLInputElement;
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await OccupationApiService.add_new_Occupation(OccupationName, OccupationNameSL || "", user.branchid);
      ...
      if (response.success) {
        ...
        Swal.fire({
          icon: "error",
          title: "Error!",
          text: response.message || "Failed to save Occupation.",
        });
      }
    } catch (err: any) {
      Swal.fire({
          ...
          <div className="max-w-7xl mx-auto space-y-8">
            ...
            <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-6 sm:p-8">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                ...
                <button
                  onClick={() => navigate("/Occupation-operations")}
            ...
            <div className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
              ...
              <div className="bg-gradient-to-r from-blue-50 to-purple-50 border-b border-gray-200 p-6">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-lg sm:text-xl font-semibold text-gray-800">Create New Occupation</h2>
                    <p className="text-sm text-gray-600">Fill in the Occupation details below</p>
                  </div>
                </div>
              </div>
              ...
              <form className="p-6 sm:p-8" onSubmit={handleSubmit}>
                <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-2">
                  ...
                  <div className="flex flex-col">
                    <label htmlFor="OccupationName" className="flex items-center gap-2 text-sm font-semibold text-gray-700 mb-2">
                    ...
                    <p className="text-xs text-gray-500 flex items-center gap-1 mt-1">
                      <FaInfoCircle />
                      Descriptive name for the Occupation (max 50 characters)
                    </p>
                  </div>

                  {/* Occupation Name SL Field */}
                  <div className="flex flex-col">
                    <label htmlFor="OccupationNameSL" className="flex items-center gap-2 text-sm font-semibold text-gray-700 mb-2">
                      <div className="w-2 h-2 bg-gradient-to-r from-emerald-500 to-teal-500 rounded-full"></div>
                      Occupation Name (In SL)
                    ...
                    <div className="relative group">
                      ...
                      <input
                        type="text"
                        id="OccupationNameSL"
                        value={OccupationNameSL}
                        onChange={handleOccupationNameSLChange}
                        maxLength={50}
                        ...
                        placeholder="Enter Occupation name in Hindi/Devanagari"
                        lang="hi"
                      />
                    </div>
                    {error ? (
