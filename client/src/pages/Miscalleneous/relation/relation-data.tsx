...
import RelationApiService, {
  RelationFilter,
  RelationResponse,
} from "../../../services/relation/relationapi";
import Swal from "sweetalert2";
import CRUDMaster from "../../../components/Location/CRUDOperations";
import RelationTable from "./relation-table";
import { useNavigate } from "react-router-dom";

// Define the async function to fetch Relations and ensure the return type is correct.
const fetchRelations = async (filter: RelationFilter) => {
  const res = await RelationApiService.fetchrelation(filter);
  return {
    // Ensure 'success' is a boolean, defaulting to false if undefined.
    success: res.success ?? false, // Ensure 'data' is a Relation array, defaulting to an empty array.
    data: res.relation ?? [], // Ensure 'totalCount' is a number, defaulting to 0.
    totalCount: res.totalCount ?? 0, // Ensure 'message' is a string, defaulting to an empty string.
    message: res.message ?? "",
  };
};

// Define the async function for adding a Relation.
const addRelation = async () => {
  ...
  <div class="swal2-form-container space-y-6 p-2">
    ...
    <div class="swal2-input-group">
      <label for="description" class="block text-sm font-semibold text-slate-700 mb-2 flex items-center gap-2">
    ...
    <!-- Relation Name SL Field -->
    <div class="swal2-input-group">
      <label for="descriptionsl" class="block text-sm font-semibold text-slate-700 mb-2 flex items-center gap-2">
      ...
      <div class="relative">
        <input 
          ...
          placeholder="Enter Relation Name SL" 
          aria-required="true"
          autocomplete="off"
          lang="si"
        >
    ...
    <div class="bg-gradient-to-r from-emerald-50 to-green-50 border border-emerald-200 rounded-lg p-4 mt-6">
      <div class="flex items-center gap-3">
        ...
        <div class="flex-1">
          <p class="text-sm font-semibold text-slate-700">Creating New Relation</p>
          <p class="text-xs text-slate-500 mt-1">Fill in all the required information to add a new Relation.</p>
        </div>
      ...
      <div class="flex items-start gap-2">
        ...
        <div class="text-xs text-slate-600">
          ...
          <ul class="list-disc list-inside mt-1 space-y-1 text-blue-600">
            <li>Relation name should be descriptive and unique</li>
...
`,
    ...
    preConfirm: () => {
      ...
      if (!description) {
        firstEmptyElement = descriptionInput;
      }
      ...
      return { description, descriptionsl: (document.getElementById("descriptionsl") as HTMLInputElement).value.trim()};
    },
  });

  if (formValues) {
    try {
      await RelationApiService.add_new_relation(
        formValues.description,
        formValues.descriptionsl || ""
      );
      Swal.fire({
        title: "Success!",
        text: "New Relation has been added.",
        icon: "success",
        timer: 1500,
        showConfirmButton: false,
      });
    } catch (err: any) {
      Swal.fire("Error!", err.message || "Failed to add Relation.", "error");
    }
  }
};

// Define the async function for modifying a Relation.
const modifyRelation = async (Relation: Relation) => {
  const { value: formValues } = await Swal.fire({
  ...
  <div class="swal2-form-container space-y-6 p-2">
    ...
    <div class="bg-gradient-to-r from-slate-50 to-blue-50 border border-slate-200 rounded-lg p-4 mt-6">
      <div class="flex items-center gap-3">
        ...
        <div class="flex-1">
          <p class="text-sm font-semibold text-slate-700">Editing Relation Information</p>
          <p class="text-xs text-slate-500 mt-1">Update the Relation details above. Fields marked with * are required.</p>
        </div>
      ...
      <div class="flex items-start gap-2">
        ...
        <div class="text-xs text-slate-600">
          ...
          <ul class="list-disc list-inside mt-1 space-y-1 text-amber-600">
            <li>Relation code changes may affect existing records</li>
            <li>Ensure Relation Name and Relation Code is unique in the system</li>
          </ul>
        </div>
      </div>
    </div>
...
`,
    showCancelButton: true,
    confirmButtonText: "Update Relation",
    confirmButtonColor: "#3085d6",
    cancelButtonColor: "#d33",
    preConfirm: () => {
      const descriptionInput = document.getElementById(
      ...
      return { relationId: Relation.relationId, description, descriptionsl: (document.getElementById("descriptionsl") as HTMLInputElement).value.trim() };
    },
  });

  if (formValues) {
    try {
      await RelationApiService.modify_relation(
        formValues.relationId,
        formValues.description,
        formValues.descriptionsl || ""
      );
      Swal.fire({
        title: "Success!",
        text: "Relation has been updated.",
        icon: "success",
        timer: 1500,
        showConfirmButton: false,
      });
...
// Define the async function for deleting a Relation.
const deleteRelation = async (Relation: Relation) => {
  const result = await Swal.fire({
    title: "Delete Relation",
    text: `Are you sure you want to delete "${Relation.description}"? This action cannot be undone.`,
    icon: "warning",
    showCancelButton: true,
    confirmButtonColor: "#3085d6",
    cancelButtonColor: "#d33",
  ...
  if (result.isConfirmed) {
    try {
      await RelationApiService.delete_relation(
        Relation.relationId,
        Relation.description,
        Relation.descriptionSL
      );
      Swal.fire({
        title: "Deleted!",
        text: `Relation "${Relation.description}" has been deleted.`,
        icon: "success",
        timer: 1500,
        showConfirmButton: false,
      });
...
// --- RelationMaster Component ---

const RelationMaster: React.FC = () => {
  const navigate = useNavigate();
  return (
    <CRUDMaster<Relation>
      fetchData={fetchRelations}
      addEntry={addRelation}
      modifyEntry={modifyRelation}
      deleteEntry={deleteRelation}
      pageTitle="Relation Operations"
      addLabel="Add Relation"
      onClose={() => navigate("/Relation")}
      searchPlaceholder="Search by name or code..."
      renderTable={(Relations, handleModify, handleDelete) => (
        <RelationTable
          Relations={Relations}
      ...
      getKey={(Relation) => Relation.relationId}
    />
  );
};
