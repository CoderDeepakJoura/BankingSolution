...
import RelationApiService from "../../../services/relation/relationapi";
...
const RelationMaster: React.FC = () => {
  ...
  const [RelationDescription, setRelationDescription] = React.useState("");
  const [RelationDescriptionSL, setRelationDescriptionSL] = React.useState("");
  const [error, setError] = React.useState("");
  const [loading, setLoading] = React.useState(false);

  const hindiRegex = /^[\u0900-\u097F\s.,!?]*$/;

  const handleRelationDescriptionSLChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const inputText = e.target.value;
    if (inputText === "" || hindiRegex.test(inputText)) {
      setRelationDescriptionSL(inputText);
      setError("");
    } else {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await RelationApiService.add_new_relation(RelationDescription, RelationDescriptionSL || "");
      ...
      if (response.success) {
        ...
        Swal.fire({
          icon: "error",
          title: "Error!",
          text: response.message || "Failed to save Relation.",
        });
      }
    } catch (err: any) {
      Swal.fire({
          ...
          <div className="max-w-7xl mx-auto space-y-8">
            ...
            <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-6 sm:p-8">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                ...
                <button
                  onClick={() => navigate("/Relation-operations")}
            ...
            <div className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
              ...
              <div className="bg-gradient-to-r from-blue-50 to-purple-50 border-b border-gray-200 p-6">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-lg sm:text-xl font-semibold text-gray-800">Create New Relation</h2>
                    <p className="text-sm text-gray-600">Fill in the Relation details below</p>
                  </div>
                </div>
              </div>
              ...
              <form className="p-6 sm:p-8" onSubmit={handleSubmit}>
                <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-2">
                  ...
                  <div className="flex flex-col">
                    <label htmlFor="RelationDescription" className="flex items-center gap-2 text-sm font-semibold text-gray-700 mb-2">
                    ...
                    <p className="text-xs text-gray-500 flex items-center gap-1 mt-1">
                      <FaInfoCircle />
                      Descriptive name for the Relation (max 50 characters)
                    </p>
                  </div>

                  {/* Relation Name SL Field */}
                  <div className="flex flex-col">
                    <label htmlFor="RelationDescriptionSL" className="flex items-center gap-2 text-sm font-semibold text-gray-700 mb-2">
                    ...
                    <div className="relative group">
                      ...
                      <input
                        type="text"
                        id="RelationDescriptionSL"
                        value={RelationDescriptionSL}
                        onChange={handleRelationDescriptionSLChange}
                        maxLength={50}
                        ...
                        placeholder="Enter Relation name in Hindi/Devanagari"
                        lang="hi"
                      />
                    </div>
                    {error ? (
