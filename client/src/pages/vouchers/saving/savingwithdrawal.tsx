...
const SavingWithdrawalVoucher: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Load credit accounts (skipped by the non-edit useEffect which bails on isEditMode)
    commonservice.general_accmasters_info(user.branchid).then(res => {
      if (res.success) setCreditAccounts(res.data ?? []);
    });
  }, []);
  ...
  useEffect(() => {
    ...
    if (voucherData.voucherDate) fetchData();
  }, [user.branchid, voucherData.voucherDate]);
  ...
  const handleDestBrChange = async (selected: any) => {
    ...
    const productsRes = await commonservice.fetch_saving_products(effectiveBrId, voucherData.voucherDate || undefined);
    setSavingProducts(productsRes.data || []);
  };
    ...
    if (productId && productId.trim() !== "") {
      try {
        ...
        const response = await commonservice.fetch_deposit_accounts(
          ...
          2,
          false,
          voucherData.voucherDate || undefined
        );
        if (response.success) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.success) {
        if (!isEditMode && response.message) await printAfterSave(response.message, 2, 3);
        await Swal.fire({
          icon: "success",
