...
const PatwarMaster: React.FC = () => {
  ...
  const [PatwarName, setPatwarName] = React.useState("");
  const [PatwarNameSL, setPatwarNameSL] = React.useState("");
  const [error, setError] = React.useState("");
  const [loading, setLoading] = React.useState(false);

  const hindiRegex = /^[\u0900-\u097F\s.,!?]*$/;

  const handlePatwarNameSLChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const inputText = e.target.value;
    if (inputText === "" || hindiRegex.test(inputText)) {
      setPatwarNameSL(inputText);
      setError("");
    } else {
  ...
  const handleReset = () => {
    ...
    setLoading(false);
    const PatwarName = document.getElementById("PatwarName") as HTMLInputElement;
    PatwarName.focus();
  };
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await PatwarApiService.add_new_Patwar(PatwarName, PatwarNameSL || "", user.branchid);
      ...
      if (response.success) {
        ...
        Swal.fire({
          icon: "error",
          title: "Error!",
          text: response.message || "Failed to save Patwar.",
        });
      }
    } catch (err: any) {
      Swal.fire({
          ...
          <div className="max-w-7xl mx-auto space-y-8">
            ...
            <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-6 sm:p-8">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                ...
                <button
                  onClick={() => navigate("/Patwar-operations")}
            ...
            <div className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
              ...
              <div className="bg-gradient-to-r from-blue-50 to-purple-50 border-b border-gray-200 p-6">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-lg sm:text-xl font-semibold text-gray-800">Create New Patwar</h2>
                    <p className="text-sm text-gray-600">Fill in the Patwar details below</p>
                  </div>
                </div>
              </div>
              ...
              <form className="p-6 sm:p-8" onSubmit={handleSubmit}>
                <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-2">
                  ...
                  <div className="flex flex-col">
                    <label htmlFor="PatwarName" className="flex items-center gap-2 text-sm font-semibold text-gray-700 mb-2">
                    ...
                    <div className="relative group">
                      ...
                      <input
                        ...
                        ref={inputRef}
                        value={PatwarName}
                        onChange={(e) => setPatwarName(e.target.value)}
                        required
                    ...
                    <p className="text-xs text-gray-500 flex items-center gap-1 mt-1">
                      <FaInfoCircle />
                      Descriptive name for the Patwar (max 50 characters)
                    </p>
                  </div>

                  {/* Patwar Name SL Field */}
                  <div className="flex flex-col">
                    <label htmlFor="PatwarNameSL" className="flex items-center gap-2 text-sm font-semibold text-gray-700 mb-2">
                    ...
                    <div className="relative group">
                      ...
                      <input
                        ...
                        id="PatwarNameSL"
                        value={PatwarNameSL}
                        onChange={handlePatwarNameSLChange}
                        maxLength={50}
                        ...
                        placeholder="Enter Patwar name in Hindi/Devanagari"
                        lang="hi"
                      />
                    </div>
                    {error ? (
