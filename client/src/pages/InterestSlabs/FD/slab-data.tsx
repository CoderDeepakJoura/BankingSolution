...
const deleteFDInterestSlab = async (
  ...
  const productName = dto.fdInterestSlabInfo?.productName || "Unknown Product";
  const applicableDate = dto.fdInterestSlabInfo?.applicableDate 
    ? new Date(dto.fdInterestSlabInfo.applicableDate).toLocaleDateString("en-IN", {
        day: "2-digit",
        month: "short",
        year: "numeric",
      })
  ...
  const result = await Swal.fire({
    ...
    text: `Are you sure you want to delete the FD interest slab for "${productName}" (${applicableDate})? This action cannot be undone.`,
    icon: "warning",
  ...
  if (result.isConfirmed) {
    try {
      const slabId = dto.fdInterestSlabInfo?.id;
      if (!slabId) {
      ...
      Swal.fire({
        title: "Deleted!",
        text: `FD interest slab for "${productName}" (${applicableDate}) has been deleted.`,
        icon: "success",
        timer: 1500,
...
const FDInterestSlabOperations: React.FC = () => {
  ...
  const handleAdd = () => {
    navigate("/fdproduct-interest-slab");
  };

  // Navigate to modify page
  const handleModify = async (dto: CombinedFDIntInfoDTO) => {
    const slabId = dto.fdInterestSlabInfo?.id;
    if (slabId) {
      const encryptedId = encryptId(slabId);
      navigate(`/fdproduct-interest-slab/${encryptedId}`);
    }
    return Promise.resolve();
  };

  return (
    <CRUDMaster<CombinedFDIntInfoDTO>
      fetchData={fetchSlabsWithBranch}
      addEntry={handleAdd}
      modifyEntry={handleModify}
      deleteEntry={(dto) => deleteFDInterestSlab(dto, user.branchid)}
      pageTitle="FD Interest Slab Operations"
      addLabel="Add FD Interest Slab"
      onClose={() => navigate("/slab-operations")}
      searchPlaceholder="Search by FD product name..."
