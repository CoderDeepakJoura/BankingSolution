...
namespace BankingPlatform.API.Controllers.Reports
{
    ...
    public class FDMaturityController : ControllerBase
    {
        private readonly FDMaturityService _svc;
        public FDMaturityController(FDMaturityService svc) => _svc = svc;
            ...
            [FromQuery] int productId = 0)
        {
            ...
            var (success, message, data) = await _svc.GetFDMaturityAsync(branchId, from, to, productId);
            return Ok(new { success, message, data });
        }
    }
}
