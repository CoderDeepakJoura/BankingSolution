...
namespace BankingPlatform.API.Controllers.Reports
{
    ...
    public class LoanDemandController : ControllerBase
    {
        private readonly LoanDemandService _svc;
        public LoanDemandController(LoanDemandService svc) => _svc = svc;
        ...
        public async Task<IActionResult> Get(
            ...
            [FromQuery] bool showPendingOnly = false)
        {
            if (branchId <= 0) return BadRequest(new { success = false, message = "Invalid branch." });
            ...
            var (success, message, data) = await _svc.GetLoanDemandAsync(branchId, from, to, productId, showPendingOnly);
            return Ok(new { success, message, data });
        }
    }
}
