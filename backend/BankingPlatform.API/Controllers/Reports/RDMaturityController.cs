...
namespace BankingPlatform.API.Controllers.Reports
{
    ...
    public class RDMaturityController : ControllerBase
    {
        private readonly RDMaturityService _svc;
        public RDMaturityController(RDMaturityService svc) => _svc = svc;
        ...
        public async Task<IActionResult> GetProducts([FromQuery] int branchId)
        {
            ...
            var (success, message, data) = await _svc.GetRDProductsAsync(branchId);
            ...
            [FromQuery] int productId = 0)
        {
            ...
            var (success, message, data) = await _svc.GetRDMaturityAsync(branchId, from, to, productId);
            return Ok(new { success, message, data });
        }
    }
}
