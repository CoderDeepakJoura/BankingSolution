...
namespace BankingPlatform.API.Controllers.Reports
{
    ...
    {
        private readonly LoanAdvancementService _svc;
        public LoanAdvancementController(LoanAdvancementService svc) => _svc = svc;
            ...
            [FromQuery] int productId = 0)
        {
            ...
            var (success, message, data) = await _svc.GetLoanAdvancementAsync(branchId, from, to, productId);
            return Ok(new { success, message, data });
        }
    }
}
