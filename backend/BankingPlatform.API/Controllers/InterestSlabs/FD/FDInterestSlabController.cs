...
namespace BankingPlatform.API.Controllers.InterestSlabs.FD
{
    [Route("api/[controller]")]
    [ApiController]
    [Authorize]
    public class FDInterestSlabController : ControllerBase
    {
        private readonly FDInterestSlabService _service;
        private readonly CommonFunctions _commonfunctions;
        public FDInterestSlabController(FDInterestSlabService service, CommonFunctions commonfunctions)
        {
            _service = service;
            _commonfunctions = commonfunctions;
        }
        [HttpPost]
        public async Task<IActionResult> CreateInterestSlab([FromBody] CombinedFDIntInfoDTO combinedFDDTO)
        {
            try
            {
            ...
            {
                await _commonfunctions.LogErrors(ex, nameof(CreateInterestSlab), nameof(FDInterestSlabController));
                return BadRequest(new ResponseDto
                {
                    Success = false,
        ...
        public async Task<IActionResult> UpdateFDInterestSlab(
            [FromRoute] int slabId,
            [FromBody] CombinedFDIntInfoDTO combinedFDDTO)
        {
            try
            {
                // Ensure the route productId matches the DTO Id
                if (combinedFDDTO.FDInterestSlabInfo != null &&
                    combinedFDDTO.FDInterestSlabInfo.Id.HasValue &&
                    combinedFDDTO.FDInterestSlabInfo.Id != slabId)
                {
                    return BadRequest(new ResponseDto
                    {
                        Success = false,
